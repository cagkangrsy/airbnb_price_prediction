...
def main():
    ...
    sidebar = st.sidebar
    property_type = sidebar.selectbox(label="Property Type",
                                      options=['Rental unit', 'Condo', 'Home', 'Hotel', 'Loft', 'Apartment',
                                               'Bed and breakfast', 'Townhouse', 'Villa', 'Vacation', 'Guest suite',
                                               'Guesthouse', 'Casa particular', 'Tiny home', 'Boat',
                                               'Room', 'Houseboat', 'Camper/rv', 'Cottage', ' Entire place', 'Bungalow',
                                               'Chalet', 'Farm', 'Cabin', 'Floor', 'Pension',
                                               'Earthen', 'Lodge', 'Hut', 'Tent', 'Dome', 'Barn', 'Castle', 'Minsu',
                                               'Tipi', 'Cave', 'Tower', 'Campsite', 'Windmill',
                                               'Island', 'Resort', 'Bus', 'Treehouse', 'Yurt', 'Holiday park', 'Ryokan',
                                               'Lighthouse', 'Religious building', 'Train',
                                               'Container', 'Trullo', 'In-law', 'Cycladic home', 'Kezhan'])
    city = st.selectbox(label="City",
                        options=['Amsterdam', 'Barcelona', 'Lyon', 'Copenhagen', 'Oslo', 'Florence', 'Porto',
                                 'Brussels', 'Berlin', 'Paris', 'Rome', 'Munich', 'Madrid', 'Stockholm', 'Prague',
                                 'Sevilla', 'Vienna', 'Zurich'], )
    room_type = sidebar.selectbox(label="Room Type",
    ...
    amenities = sidebar.multiselect(label="Amenities",
                                    options=["Wifi", "TV", "Kitchen", "Washer", "Free parking", "Paid parking",
                                             "Air conditioning", "Dedicated workspace", "Pool", "Hot tub", "Patio",
                                             "BBQ Grill", "Outdoor dining area", "Fire pit", "Pool table",
                                             "Indoor fireplace", "Piano", "Exercise equipment", "Lake access",
                                             "Beach access", "Ski", "Outdoor shower", "Smoke alarm", "First aid kit",
                                             "Fire extinguisher", "Lock on bedroom door", "Carbon monoxide alarm"])
    instant_bookable = sidebar.selectbox(label="Is the place instant bookable?",
                                         help="Should guests need an approval for the reservation?",
                                         options=["Yes", "No"])
    min_night = sidebar.number_input(label='Minimum nights for the stay', help="Maximum value is 365 nights", value=1,
                                     step=1, min_value=1, max_value=365)
    max_night = sidebar.number_input(label='Maximum nights for the stay', help="Maximum value is 1125 nights", value=1,
                                     step=1, min_value=1, max_value=1125)
    availability = sidebar.slider(label='Avalability in the following 30 days',
                                  help="How many days is the place available for booking in the next 30 days ", value=1,
